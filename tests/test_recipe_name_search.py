...
class TestRecipeNameSearch:
    ...
    async def test_search_recipes_by_name_exact_match(self, test_client_with_data):
        ...
        if all_data["recipes"]:
            ...
            found_recipe = next(
                (r for r in data["recipes"] if r["name"] == recipe_name), None
            )
            assert found_recipe is not None, (
                f"Should find recipe with exact name '{recipe_name}'"
            )
    ...
    async def test_search_recipes_by_name_whitespace_trimming(
        ...
        if all_data["recipes"]:
            ...
            assert (
                ...
                == clean_data["pagination"]["total_count"]
            )

    async def test_search_recipes_by_name_special_characters(
        self, test_client_with_data
    ):
        client, app = test_client_with_data
    ...
    async def test_search_recipes_name_vs_no_filter_difference(
        self, test_client_with_data
    ):
        client, app = test_client_with_data
        ...
        if all_data["recipes"]:
            ...
            assert (
                name_data["pagination"]["total_count"]
