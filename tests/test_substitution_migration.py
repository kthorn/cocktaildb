...
class TestSubstitutionMigration:
    ...
    def test_migration_adds_column(self):
        ...
        try:
            ...
            assert "substitution_level" not in columns, (
                "substitution_level should not exist before migration"
            )
            ...
            migration_path = (
                ...
                / "migrations"
                / "07_migration_add_substitution_level_to_ingredients.sql"
            )
            ...
            if migration_path.exists():
                ...
                assert "substitution_level" in columns, (
                    "substitution_level should exist after migration"
                )

                # Verify default values were set correctly
                cursor.execute(
                    "SELECT name, substitution_level FROM ingredients ORDER BY id"
                )
                ...
                for name, sub_level in results:
                    if name in expected_values:
                        assert sub_level == expected_values[name], (
                            f"{name} should have substitution_level {expected_values[name]}, got {sub_level}"
                        )
    ...
    def test_migration_preserves_existing_data(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT id, name, description, parent_id, path, created_by FROM ingredients ORDER BY id"
            )
            ...
            if migration_path.exists():
                ...
                cursor.execute(
                    "SELECT id, name, substitution_level FROM ingredients ORDER BY id"
                )
                ...
                for id_, name, sub_level in sub_level_data:
                    ...
                    assert isinstance(sub_level, int), (
                        f"substitution_level should be integer for {name}"
                    )
                    assert sub_level >= 0, (
                        f"substitution_level should be non-negative for {name}"
                    )
            ...
            if migration_path.exists():
                ...
                try:
                    ...
                    if "duplicate column name" in str(e).lower():
                        print(
                            "Migration correctly fails on duplicate column (expected behavior)"
                        )
                    else:
                        raise
