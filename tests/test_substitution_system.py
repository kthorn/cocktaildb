...
class TestIngredientSubstitution:
    ...
    def test_allow_substitution_creation_and_retrieval(self, db: Database):
        ...
        assert (
            ingredient["allow_substitution"] == 1
        )  # SQLite stores booleans as integers

        # Retrieve and verify
        retrieved = db.get_ingredient(ingredient["id"])
        assert (
            retrieved["allow_substitution"] == 1
        )  # SQLite stores booleans as integers
        ...
        assert (
            no_sub_ingredient["allow_substitution"] == 0
        )  # SQLite stores booleans as integers
        ...
        assert (
            child_ingredient["allow_substitution"] == 1
        )  # SQLite stores booleans as integers
    ...
    def test_allow_substitution_update(self, db: Database):
        ...
        updated = db.update_ingredient(ingredient["id"], {"allow_substitution": False})

        assert updated["allow_substitution"] == 0  # SQLite stores booleans as integers

        # Update back to True
        updated = db.update_ingredient(ingredient["id"], {"allow_substitution": True})
    ...
    def test_mixed_substitution_levels_in_recipe(self, db: Database):
        ...
        db.add_user_ingredient(
            user_id, bourbons["makers_mark"]["id"]
        )  # Different bourbon - should work
        db.add_user_ingredient(
            user_id, amaros["montenegro"]["id"]
        )  # Different amaro - won't work
...
class TestSubstitutionAPI:
    ...
    def test_parent_child_no_substitution_blocks_match(self, db: Database):
        ...
        recipe = db.create_recipe(
            {
                ...
                "created_by": "test-user",
                "ingredients": [
                    {"ingredient_id": old_tom_gin["id"], "amount": 2.0, "unit_id": 1}
                ],
            }
        )
        ...
        assert len(results) == 0, (
            "Recipe should NOT match when user has parent but child doesn't allow substitution"
        )
    ...
    def test_blocking_middle_parent_prevents_match(self, db: Database):
        ...
        recipe = db.create_recipe(
            {
                ...
                "created_by": "test-user",
                "ingredients": [
                    {"ingredient_id": wray_nephew["id"], "amount": 1.0, "unit_id": 1}
                ],
            }
        )
        ...
        assert len(results) == 0, (
            "Recipe should NOT match when blocking parent exists in substitution path"
        )
