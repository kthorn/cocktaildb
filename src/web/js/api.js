...
export class CocktailAPI {
    ...
    async createIngredient(ingredientData) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can create ingredients.',
            );
        }
    ...
    async updateIngredient(id, ingredientData) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can update ingredients.',
            );
        }
    ...
    async deleteIngredient(id) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can delete ingredients.',
            );
        }
    ...
    async bulkUploadRecipes(recipesData) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can bulk upload recipes.',
            );
        }
    ...
    async bulkUploadIngredients(ingredientsData) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can bulk upload ingredients.',
            );
        }
    ...
    async bulkUploadIngredientValues(csvText) {
        if (!this.isEditor()) {
            throw new Error(
                'Editor access required. Only editors and admins can update ingredient values.',
            );
        }
    ...
    async addTagToRecipe(recipeId, tagName, jsTagType) {
        ...
        if (jsTagType === 'private') {
            ...
            const existingTag = privateTags.find(
            ...
            const existingTag = publicTags.find(
                (tag) => tag.name.toLowerCase() === tagName.toLowerCase(),
            );
