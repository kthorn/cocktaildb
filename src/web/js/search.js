...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function performSearch(reset = true) {
        ...
        try {
            ...
            if (result && result.recipes) {
                ...
                totalSearchPages = result.pagination.has_next
                    ? currentSearchPage + 1
                    : currentSearchPage;
        ...
        try {
            ...
            if (api.isAuthenticated()) {
                try {
                    ...
                    console.warn(
                        'Could not load private tags (user may not be authenticated):',
                        error,
                    );
                }
            }
    ...
    function setupIngredientRow(row) {
        ...
        ingredientSearchInput.addEventListener('keydown', (e) => {
            ...
            activeRowIndex = Array.from(
                ingredientQueryBuilder.querySelectorAll('.item-row'),
            ).indexOf(row);
    ...
    async function searchTags(query) {
        ...
        try {
            ...
            const filteredResults = results.filter(
                (tag) =>
                    !selectedTags.some(
                        (selected) => selected.name === tag.name && selected.type === tag.type,
                    ),
            );

            if (filteredResults.length === 0) {
                dropdown.innerHTML =
                    '<div class="no-results">All matching tags already selected</div>';
                return;
            }
    ...
    function handleNameSuggestKeydown(e) {
        ...
        switch (e.key) {
            case 'ArrowDown':
                e.preventDefault();
                activeNameSuggestionIndex = Math.min(
                    activeNameSuggestionIndex + 1,
                    items.length - 1,
                );
    ...
    function updateAuthDependentUI() {
        ...
        if (useUserRatingCheckbox) {
            ...
            if (!authenticated) {
                ...
                useUserRatingCheckbox.title =
                    'Filter by your personal ratings instead of average ratings';
            }
        }
