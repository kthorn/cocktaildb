...
class Settings(BaseSettings):
    ...
    user_pool_id: str = Field(
        default="", description="Cognito User Pool ID", env="USER_POOL_ID"
    )
    app_client_id: str = Field(
        default="", description="Cognito App Client ID", env="APP_CLIENT_ID"
    )
    ...
    base_url: str = Field(
        default="https://mixology.tools", description="Public base URL for the site"
    )
    ...
    def __init__(self, **kwargs):
        ...
        logger.info(
            f"Config initialized - user_pool_id: {self.user_pool_id[:10] + '...' if self.user_pool_id else 'MISSING'}, app_client_id: {self.app_client_id[:10] + '...' if self.app_client_id else 'MISSING'}"
        )
