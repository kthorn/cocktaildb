...
- Document build/deploy/artifact reuse, frontend-only Vite development and optional local FastAPI integration, config generation, first rollout, recovery and cleanup. Explicitly note the accepted one-time first-deployment refresh requirement. Explicitly state that tabs older than the retained release window may require refresh.
